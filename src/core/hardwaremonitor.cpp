...
void HardwareMonitor::checkAvailability()
{
    ...
    process.waitForFinished(HardwareConstants::PROCESS_TIMEOUT_MS);
...
CpuTemperature HardwareMonitor::parseCpuTemp(const QString &sensorsOutput)
{
    ...
    for (const QString &line : lines) {
        ...
        if (match.hasMatch()) {
            int coreNum = match.captured(1).section(':', 0, 0).remove("Core").trimmed().toInt();
            cpu.cores[coreNum] = match.captured(1).toDouble();
        }
    }
    
    // 尝试从 sysfs hwmon 读取（支持更多架构）
    if (!cpu.isValid) {
        ...
        for (const QString &hwmon : hwmonList) {
            ...
            if (nameFile.open(QIODevice::ReadOnly)) {
                ...
                // CPU 相关的 hwmon（扩展支持国产CPU）
                // x86: coretemp, k10temp, k8temp
                // ARM: cpu_thermal, thermal_zone, soc_thermal
                // 龙芯: loongson_thermal, ls_thermal
                // 飞腾: phytium_thermal, ft_thermal
                // 华为鲲鹏: tsens, hisi_thermal
                if (name.contains("coretemp") || name.contains("k10temp") || 
                    name.contains("k8temp") || name.contains("cpu_thermal") ||
                    name.contains("soc_thermal") || name.contains("loongson") ||
                    name.contains("ls_thermal") || name.contains("phytium") ||
                    name.contains("ft_thermal") || name.contains("tsens") ||
                    name.contains("hisi_thermal") || name.contains("thermal_zone")) {
                    ...
                    if (tempFile.open(QIODevice::ReadOnly)) {
                        ...
                        double temp = tempStr.toDouble() / HardwareConstants::TEMP_SCALE_FACTOR;
                        if (temp > HardwareConstants::MIN_REASONABLE_TEMP && temp < HardwareConstants::MAX_CPU_TEMP) {
                            cpu.package = temp;
                            cpu.isValid = true;
                            tempFile.close();
                            break;
    ...
    if (!cpu.isValid) {
        ...
        for (const QString &thermal : thermalList) {
            ...
            if (typeFile.open(QIODevice::ReadOnly)) {
                ...
                // 查找 CPU 相关的 thermal zone
                // x86: x86_pkg_temp, acpitz
                // ARM: cpu-thermal, soc-thermal
                // 龙芯: loongson-thermal
                // 飞腾: phytium-thermal
                // 华为鲲鹏: tsens_tz_sensor
                if (type.contains("cpu") || type.contains("pkg_temp") || 
                    type.contains("soc") || type.contains("loongson") ||
                    type.contains("phytium") || type.contains("tsens") ||
...
BoardTemperature HardwareMonitor::parseBoardTemp(const QString &sensorsOutput)
{
    ...
    if (!board.isValid) {
        ...
        for (const QString &hwmon : hwmonList) {
            ...
            if (nameFile.open(QIODevice::ReadOnly)) {
                ...
                // 主板相关的 hwmon（扩展支持）
                // x86 主板芯片: nct, it87, f718, w836
                // 品牌主板: asus, gigabyte, dell, hp, msi, asrock
                // ARM/国产平台: soc_thermal, board_thermal, ambient
                // 龙芯: ls7a_thermal (龙芯7A桥片)
                // 飞腾: phytium_board
                if (name.contains("nct") || name.contains("it87") || 
                    ...
                    name.contains("msi") || name.contains("asrock") ||
                    name.contains("board") || name.contains("ambient") ||
                    name.contains("ls7a") || name.contains("phytium")) {
                    ...
                    for (int i = 2; i <= 5; ++i) {
                        ...
                        if (tempFile.open(QIODevice::ReadOnly)) {
                            ...
                            if (temp > 0 && temp < 100) {  // 合理范围
                                ...
                                board.label = QString("%1 temp%2").arg(name).arg(i);
                                board.isValid = true;
                                tempFile.close();
                                break;
                            }
    ...
    // 尝试从 thermal_zone 读取 (ACPI 热区 + ARM/国产平台)
    if (!board.isValid) {
        QDir thermalDir("/sys/class/thermal");
        QStringList thermalList = thermalDir.entryList(QDir::Dirs | QDir::NoDotAndDotDot);
        ...
        for (const QString &thermal : thermalList) {
            ...
            if (typeFile.open(QIODevice::ReadOnly)) {
                ...
                // x86: pch (Platform Controller Hub), board, sys, ambient
                // ARM: soc-thermal, board-thermal
                // 龙芯: ls7a-thermal
                // 飞腾: phytium-board
                if (type.contains("pch") || type.contains("board") || 
                    type.contains("sys") || type.contains("ambient") ||
                    type.contains("soc") || type.contains("ls7a") ||
                    type.contains("phytium")) {
...
QList<GpuTemperature> HardwareMonitor::getAmdGpuTemps()
{
    ...
    for (const QString &card : cardList) {
        ...
        // 读取显卡驱动名称，用于识别国产GPU
        QString driverPath = QString("/sys/class/drm/%1/device/driver").arg(card);
        QFileInfo driverInfo(driverPath);
        QString driverName = driverInfo.isSymLink() ? driverInfo.symLinkTarget().split('/').last() : "";
        ...
        for (const QString &hwmon : hwmonList) {
            ...
            if (tempFile.open(QIODevice::ReadOnly)) {
                ...
                if (temp > 0 && temp < 150) {  // 合理温度范围
                    GpuTemperature gpu;
                    // 根据驱动名称识别GPU类型
                    if (driverName.contains("amdgpu")) {
                        gpu.name = QString("AMD GPU (%1)").arg(card);
                    } else if (driverName.contains("i915") || driverName.contains("xe")) {
                        gpu.name = QString("Intel GPU (%1)").arg(card);
                    } else if (driverName.contains("jm") || driverName.contains("jingjiamicro")) {
                        gpu.name = QString("景嘉微 GPU (%1)").arg(card);
                    } else if (driverName.contains("mthreads") || driverName.contains("musa")) {
                        gpu.name = QString("摩尔线程 GPU (%1)").arg(card);
                    } else if (driverName.contains("loongson") || driverName.contains("lsdc")) {
                        gpu.name = QString("龙芯显卡 (%1)").arg(card);
                    } else {
        ...
        // 尝试直接读取驱动温度 (适用于部分显卡)
        if (gpus.isEmpty()) {
            QString gpuTempPath = QString("/sys/class/drm/%1/device/hwmon/hwmon*/temp1_input").arg(card);
            QProcess findProcess;
            findProcess.start("sh", QStringList() << "-c" << "cat " + gpuTempPath + " 2>/dev/null | head -1");
            ...
            if (!output.isEmpty()) {
                ...
                if (temp > 0 && temp < 150) {
                    ...
                    if (driverName.contains("amdgpu")) {
                        gpu.name = QString("AMD GPU (%1)").arg(card);
                    } else if (driverName.contains("jm")) {
                        gpu.name = QString("景嘉微 GPU (%1)").arg(card);
                    } else if (driverName.contains("mthreads")) {
                        gpu.name = QString("摩尔线程 GPU (%1)").arg(card);
                    } else {
                        gpu.name = QString("GPU (%1)").arg(card);
                    }
    ...
    // 通过 sensors 命令获取 GPU 温度 (备用方案，支持更多类型)
    if (gpus.isEmpty() && m_sensorsAvailable) {
        ...
        // 匹配各种 GPU 传感器的温度
        // amdgpu, radeon, i915, jm, mthreads, loongson
        QRegularExpression gpuTempRe(R"((amdgpu|radeon|i915|xe|jm|mthreads|loongson|edge|junction).*?:\s*([0-9.]+))");
        ...
        while (matchIter.hasNext()) {
            ...
            if (temp > 0 && temp < 150) {
                GpuTemperature gpu;
                QString sensorType = match.captured(1).toLower();
                if (sensorType.contains("amd")) {
                    gpu.name = "AMD GPU";
                } else if (sensorType.contains("i915") || sensorType.contains("xe")) {
                    gpu.name = "Intel GPU";
                } else if (sensorType.contains("jm")) {
                    gpu.name = "景嘉微 GPU";
                } else if (sensorType.contains("mthreads")) {
                    gpu.name = "摩尔线程 GPU";
                } else if (sensorType.contains("loongson")) {
                    gpu.name = "龙芯显卡";
                } else {
                    gpu.name = "GPU";
                }
...
QList<DiskTemperature> HardwareMonitor::getDiskTemperatures()
{
    ...
    for (const QString &block : blockList) {
        ...
        if (!disk.isValid && m_smartctlAvailable) {
            ...
            process.waitForFinished(HardwareConstants::LONG_PROCESS_TIMEOUT_MS);
            QString output = QString::fromUtf8(process.readAllStandardOutput());
