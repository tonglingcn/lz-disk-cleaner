...
void ResourcesWidget::initUI()
{
    ...
    // 磁盘信息表格双击事件 - 显示分区详情
    connect(m_diskInfoTable, &QTableWidget::cellDoubleClicked,
            this, &ResourcesWidget::onDiskInfoCellDoubleClicked);
    ...
    // 硬盘结构表格双击事件 - 显示分区详情
    connect(m_diskStructureTable, &QTableWidget::cellDoubleClicked,
            this, &ResourcesWidget::onDiskStructureCellDoubleClicked);
...
void ResourcesWidget::onDiskInfoCellDoubleClicked(int row, int column)
{
    Q_UNUSED(column);
    QTableWidgetItem *mountItem = m_diskInfoTable->item(row, 6);
    if (!mountItem) return;

    QString mountPath = mountItem->text();
    if (mountPath.isEmpty()) return;

    QString device = m_diskInfoTable->item(row, 0)->text();
    showDiskDetailDialog(mountPath, tr("%1 - 目录详情").arg(device));
}

void ResourcesWidget::onDiskStructureCellDoubleClicked(int row, int column)
{
    Q_UNUSED(column);
    QTableWidgetItem *mountItem = m_diskStructureTable->item(row, 4);
    if (!mountItem) return;

    QString mountPoint = mountItem->text();
    if (mountPoint == "-" || mountPoint.isEmpty()) {
        // 没有挂载点的设备（如磁盘本身），尝试找其子分区
        LOG_INFO(QString("Device %1 has no mount point, skipping detail").arg(
            m_diskStructureTable->item(row, 0)->text()));
        return;
    }

    QString deviceName = m_diskStructureTable->item(row, 0)->text();
    showDiskDetailDialog(mountPoint, tr("%1 - %2").arg(deviceName).arg(tr("目录详情")));
}

void ResourcesWidget::showDiskDetailDialog(const QString &mountPath, const QString &title)
{
    LOG_INFO(QString("Showing disk detail for: %1").arg(mountPath));
    ...
    QLabel *pathLabel = new QLabel(tr("路径: %1").arg(mountPath), dialog);
    pathLabel->setStyleSheet("color: #666; margin-bottom: 10px;");
    mainLayout->addWidget(pathLabel);

    QTableWidget *table = new QTableWidget(dialog);
    table->setColumnCount(3);
    table->setHorizontalHeaderLabels(
        QStringList() << tr("目录/文件") << tr("大小") << tr("类型")
    );
    ...
    QPushButton *openBtn = new QPushButton(tr("打开所在目录"), dialog);
    ...
    connect(openBtn, &QPushButton::clicked, this, [this, mountPath]() {
    ...
    QList<QPair<QString, qint64>> entries = scanDirectorySizes(mountPath, 2);
    ...
    for (int i = 0; i < entries.size(); ++i) {
        ...
        nameItem->setToolTip(entry.first);
        nameItem->setData(Qt::UserRole, entry.first);  // 存储完整路径
        table->setItem(i, 0, nameItem);

        QTableWidgetItem *sizeItem = new QTableWidgetItem(formatSize(entry.second, true));
        sizeItem->setTextAlignment(Qt::AlignRight | Qt::AlignVCenter);
        table->setItem(i, 1, sizeItem);

        // 判断是文件还是目录
        QFileInfo fi(entry.first);
        QString typeText = fi.isDir() ? tr("目录") : tr("文件");
        QTableWidgetItem *typeItem = new QTableWidgetItem(typeText);
        typeItem->setTextAlignment(Qt::AlignCenter);
        typeItem->setForeground(fi.isDir() ? QBrush(QColor(52, 152, 219)) : QBrush(QColor(149, 165, 166)));
    ...
    QLabel *summaryLabel = new QLabel(
        tr("共 %1 项，总大小: %2").arg(entries.size()).arg(formatSize(totalSize, true)),
        dialog
    );
    summaryLabel->setAlignment(Qt::AlignRight | Qt::AlignVCenter);
    summaryLabel->setStyleSheet("color: #666; padding-top: 8px; border-top: 1px solid #ecf0f1;");
...
QList<QPair<QString, qint64>> ResourcesWidget::scanDirectorySizes(const QString &dirPath, int maxDepth)
{
    QList<QPair<QString, qint64>> results;
    ...
    QFileInfoList entries = dir.entryInfoList(
        ...
        QDir::DirsFirst | QDir::Name | QDir::Reversed
    );
    ...
    for (const QFileInfo &info : entries) {
        ...
        if (info.isDir()) {
            // 使用 du -sb 获取目录大小
            QProcess duProc;
            duProc.start("du", QStringList() << "-sb" << info.absoluteFilePath());
            duProc.waitForFinished(5000);
            QByteArray output = duProc.readAllStandardOutput().split('\t')[0];
            qint64 size = output.toLongLong();

            results.append(qMakePair(info.absoluteFilePath(), size));
        } else if (info.isFile()) {
            results.append(qMakePair(info.absoluteFilePath(), info.size()));
        }
    }
