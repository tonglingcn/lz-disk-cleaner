...
void SystemSlimmerWidget::onDuplicateGroupSelected()
{
    ...
    for (int i = 0; i < m_duplicateTable->rowCount(); ++i) {
        QWidget *checkWidget = m_duplicateTable->cellWidget(i, 1);
        QCheckBox *checkBox = checkWidget ? checkWidget->findChild<QCheckBox*>() : nullptr;
        if (checkBox && checkBox->isChecked()) {
            // 从表格中获取文件路径（存储在 UserRole 中）
            QTableWidgetItem *pathItem = m_duplicateTable->item(i, 2);
            if (pathItem) {
                QString path = pathItem->data(Qt::UserRole).toString();
                QFileInfo info(path);
                if (info.exists()) {
                    m_selectedDuplicateTotalSize += info.size();
                }
            }
        }
    }
    ...
    // 更新删除按钮状态和提示文本
    bool hasSelection = m_selectedDuplicateTotalSize > 0;
    if (m_deleteDuplicateBtn) {
        m_deleteDuplicateBtn->setEnabled(hasSelection);
    }
    if (m_trashDuplicateBtn) {
        m_trashDuplicateBtn->setEnabled(hasSelection);
    }
    
    // 更新状态栏显示
    if (m_duplicateSizeLabel && hasSelection) {
        m_duplicateSizeLabel->setText(tr("已选中: %1").arg(formatFileSize(m_selectedDuplicateTotalSize)));
    }
}
